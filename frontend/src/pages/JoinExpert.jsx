...
import { isValidPhoneNumber, getCountries, getCountryCallingCode } from 'libphonenumber-js/min';
...
const JoinExpert = () => {
            ...
            try {
                ...
                if (!isValidPhoneNumber(fullNumber, selectedCountry)) {
                    ...
                    if (selectedCountry === 'IN' && val.length !== 10) {
                        setPhoneError("In India, the mobile number must be exactly 10 digits long.");
                    } else {
    ...
    const onSubmit = (data) => {
        // Re-verify phone before final submit to be strictly safe
        ...
        <div className="form-container">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="form-group">
                    ...
                    {phoneError && <div className="error-text" style={{ position: 'relative', marginTop: '5px' }}>{phoneError}</div>}
                </div>

                <div className="form-group" style={{ marginTop: '30px' }}>
                    <label>LinkedIn Profile URL *</label>
                    <input className="form-control" placeholder="https://linkedin.com/in/..." {...register("linkedin", { required: "LinkedIn URL is required to verify your profile" })} />
                    {errors.linkedin && <span className="error-text">{errors.linkedin.message}</span>}
                </div>
                ...
                <div className="form-group">
                    ...
                    <input type="number" className="form-control" min="0" {...register("experience", {
                        ...
                        min: { value: 5, message: "We typically require senior professionals with 5+ years experience." }
                    })} />
                ...
                <div className="form-group">
                    ...
                    <input type="file" className="form-control" accept=".pdf,.doc,.docx" {...register("resume", { required: "Resume is required for profile vetting" })} />
                    <small style={{ color: '#666', marginTop: '5px', display: 'block' }}>Max file size: 5MB</small>
                    {errors.resume && <span className="error-text">{errors.resume.message}</span>}
                </div>

                <div style={{ textAlign: 'center', marginTop: '40px' }}>
                    <button type="submit" className="btn-primary" style={{ width: '100%' }}>SUBMIT PROFILE FOR VERIFICATION</button>
                </div>
            </form>
        </div>
    );
